...
void Scene::buildBVH(std::string method) {
    printf(" - Generating BVH...\n\n");
    if(method == "NAIVE")
        this->bvh = new BVHAccel(objects, 1, "NAIVE");
    else this->bvh = new BVHAccel(objects, 1, "SAH");
}

Intersection Scene::intersect(const Ray &ray) const//intersect !!!!!!!!!!!!!!!!!!!
{
    return this->bvh->Intersect(ray);
}

bool Scene::trace(// kan yi xia zhe ge guang xian dao di da shang le shen me obj
        const Ray &ray,
...
Vector3f Scene::castRay(const Ray &ray, int depth) const
{
    if (depth > this->maxDepth) {
        return Vector3f(0.0,0.0,0.0);// jiu dai biao mei you da shang dong xi
    }
